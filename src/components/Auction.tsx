...
import { addUnsoldPlayerToUnsoldList } from "../features/unsoldPlayer/unsoldPlayerSlice";
...
const Auction = () => {
  ...
  const handleSold = () => {
    if (!pickedName) return;
    dispatch(deletePlayerFromAuction(pickedName.id));
    setPickedName(null);
  };

  //delete player from auction and mark/store as unsold
  const handleUnsold = () => {
    dispatch(deletePlayerFromAuction(pickedName!.id));
    dispatch(addUnsoldPlayerToUnsoldList(pickedName!));
    setPickedName(null);
  };
